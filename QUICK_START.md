...
- Use the dedicated devnet config and launcher to exercise discovery and sign real devnet swaps without touching mainnet:
  ...
  - Fund the active wallet on devnet:
    - If you have Solana CLI: `solana config set --url https://devnet.helius-rpc.com/?api-key=demo-helius-key && solana airdrop 2` (re-run until balance ≥ 1 SOL)
...
- Notes
  ...
  - Ensure `SOLANA_TESTNET_RPC_URL` points to `https://devnet.helius-rpc.com/?api-key=demo-helius-key`.
...
```bash
export SOLANA_RPC_URL=https://mainnet.helius-rpc.com/?api-key=demo-helius-key
