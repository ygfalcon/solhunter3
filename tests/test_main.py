...
def _call_iteration(mem, pf, **kwargs):
    state = kwargs.pop("state", None) or main_module.TradingState()
    asyncio.run(main_module._run_iteration(mem, pf, state, **kwargs))

try:
    import torch.nn.utils.rnn  # type: ignore[attr-defined]
except Exception:  # pragma: no cover - torch optional for tests
    _rnn = types.SimpleNamespace(
        pad_sequence=lambda seqs, batch_first=False: list(seqs),
        PackedSequence=object,
    )
    torch_nn_utils = types.SimpleNamespace(rnn=_rnn)
    torch_nn = types.SimpleNamespace(utils=torch_nn_utils)
    torch_stub = types.SimpleNamespace(nn=torch_nn)
...
if "solhunter_zero.rl_training" not in sys.modules:
    class _DummyMultiAgentRL:
        def train_controller(self, agents):
            return {name: 1.0 for name in agents}

    sys.modules["solhunter_zero.rl_training"] = types.SimpleNamespace(
        MultiAgentRL=_DummyMultiAgentRL,
    )
...
def test_run_iteration_sells(monkeypatch):
    ...
    _call_iteration(mem, pf, dry_run=True)
...
def test_run_iteration_stop_loss(monkeypatch):
    ...
    _call_iteration(mem, pf, dry_run=True, stop_loss=0.1)
...
def test_run_iteration_take_profit(monkeypatch):
    ...
    _call_iteration(mem, pf, dry_run=True, take_profit=0.1)
...
def test_run_iteration_agent_manager(monkeypatch):
    ...
    _call_iteration(mem, pf, dry_run=True, agent_manager=DummyManager())
