...
def test_run_auto_restores_keypair_path(monkeypatch, tmp_path):
    with monkeypatch.context() as mp:
        ...
        mp.setitem(sys.modules, "sklearn.linear_model", types.SimpleNamespace(LinearRegression=object))
        mp.setitem(
            sys.modules,
            "sklearn.ensemble",
        ...
        monkeypatch.setattr(main_module.wallet, "KEYPAIR_DIR", str(tmp_path))
        (tmp_path / "kp.json").write_text("{}")
        ...
        def fake_main(**kw):
            captured["during"] = os.environ.get("KEYPAIR_PATH")
        ...
        mp.setenv("KEYPAIR_PATH", "original.json")
        captured.clear()
        main_module.run_auto(iterations=1, dry_run=True)
        assert captured["during"] == "original.json"
        assert os.environ["KEYPAIR_PATH"] == "original.json"

        mp.delenv("KEYPAIR_PATH", raising=False)
        captured.clear()
        expected = os.path.join(str(tmp_path), "kp.json")
        main_module.run_auto(iterations=1, dry_run=True)
        assert captured["during"] == expected
        assert "KEYPAIR_PATH" not in os.environ
