...
def test_emit_ui_ready_uses_detected_host(monkeypatch, caplog, tmp_path):
    ...
    monkeypatch.setenv("UI_HTTP_URL", "http://0.0.0.0:5000")
...
def test_emit_ui_ready_prefers_canonical_http_url(monkeypatch, caplog, tmp_path):
    caplog.set_level(logging.INFO)

    monkeypatch.setenv("UI_HTTP_URL", "https://runtime.canonical.test:8443")
    captured_urls: list[str] = []
    monkeypatch.setattr(
        "solhunter_zero.runtime.orchestrator._publish_ui_url_to_redis",
        lambda url: captured_urls.append(url),
    ...
    orch = RuntimeOrchestrator(run_http=False)
    orch._emit_ui_ready("127.0.0.1", 5000)

    readiness_logs = [message for message in caplog.messages if message.startswith("UI_READY ")]
    assert readiness_logs, "expected UI_READY log entry"
    assert any(
        "url=https://runtime.canonical.test:8443" in message for message in readiness_logs
    )
    assert captured_urls == ["https://runtime.canonical.test:8443"]
    assert (
        (tmp_path / "ui_url.txt").read_text(encoding="utf-8")
        == "https://runtime.canonical.test:8443"
    )
