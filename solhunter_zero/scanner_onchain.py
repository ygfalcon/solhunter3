...
from .http import STATIC_DNS_MAP
...
async def scan_tokens_onchain(
    ...
    *,
    ...
    # Use bounded concurrency so we do not stall the pipeline waiting on sequential RPC calls.
    try:
    ...
    async def _collect_metrics(mint: str) -> Dict[str, Any]:
    ...
    tasks = [asyncio.create_task(_collect_metrics(mint)) for mint in uniq_mints]
